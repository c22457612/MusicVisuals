...
public class SoundWave {
    private MyVisual mv; // Reference to the PApplet for drawing
    ...
    // Constructor
    public SoundWave(MyVisual _mv, FFT fft, float waveHeight, boolean[] modes, boolean extremeColour,
            boolean startDrawingShapes, AudioPlayer player, AudioPlayer song, float width, float height,
            int frameCount) {
        this.mv = _mv;
        this.fft = fft;
    ...
    public void render() {
        ...
        float thicknessTop = 0.5f;
        float thicknessBottom = 0.5f;
        ...
        for (float y = 0; y <= waveLength; y += 0.1) { // top left pyramid
            ...
            float x = PApplet.sin((this.height - y) * waveFrequency + this.frameCount * 0.05f) * amplitude
                    * maxAmplitude;
            ...
            mv.line(x, y, x, y - thicknessTop); // Draw points upward for a vertical wave
        }
        ...
        for (float y = this.height; y >= this.height - waveLength; y -= 0.1) {
            ...
            float amplitude = this.fft.getAvg(index) * this.waveHeight * bottomWaveAmplitudeScale; // Apply the scale
                                                                                                   // here
            float x = PApplet.sin(((this.height - y) * waveFrequency - this.frameCount * 0.2f)) * amplitude
                    * maxAmplitude;
            x += width / 4.1f;
            ...
            mv.line(x, y, x, y + thicknessBottom);
        }
